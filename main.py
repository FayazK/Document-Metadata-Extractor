...
* **Keywords:** Relevant keywords or tags describing the document's content.
...
"""
    ...
    def extract_metadata(self, text: str, file_path: str, file_format: str) -> Tuple[Optional[Dict[str, Any]], Optional[Dict[str, Any]]]:
            ...
            if field in metadata:
                ...
                if isinstance(value, dict):
                    if "#text" in value:
                        ...
                        # Join all non-dict values
                        parts = []
                        for k, v in value.items():
                            if not isinstance(v, dict) and not isinstance(v, list):
                                parts.append(f"{k}: {v}")
                        row_data.append(", ".join(parts) if parts else "Complex structure")
                    ...
                    if all(isinstance(item, str) for item in value):
                        ...
                        for item in value:
                            if isinstance(item, dict) and "#text" in item:
                                ...
                                parts.append(item)
                        row_data.append(", ".join(parts) if parts else "Complex list")
...
class MetadataProcessor:
    ...
    def process_directory(self, directory_path: str, output_filename: str = "metadata_output.xlsx") -> None:
        ...
        for i, file_path in enumerate(files):
            ...
            # Extract metadata
            metadata, raw_response = self.metadata_extractor.extract_metadata(extracted_text, file_path_str, file_format)
...
def main():
    ...
    parser.add_argument("--output-dir", default="output", help="Directory to save JSON responses and other output files")
